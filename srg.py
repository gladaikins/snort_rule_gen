...
def cmd():
    ...
    parser = argparse.ArgumentParser(
        description="Easily retrain my text-generating model on new texts. (https://github.com/akings/aiking)"
    )
    ...
    parser.add_argument(
        '--model_name',  help="[finetune] Name of the vocab model to finetune",
        nargs='?', default='124M')
    parser.add_argument(
...
def cmd_finetune(dataset, run_name, checkpoint_dir, model_name, model_dir, steps,
    ...
    if not is_gpt2_downloaded(model_dir=model_dir, model_name=model_name):
        download_vocab(model_dir=model_dir, model_name=model_name)
